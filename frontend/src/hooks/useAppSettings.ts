...
export function useAppSettings() {
  ...
  const settings: AppSettings = record
    ? { title: record.title, icon: record.icon }
    : DEFAULT_SETTINGS;

  const fetchSettings = useCallback(async () => {
    try {
      setLoading(true);
      const records = await pb.collection('app_settings').getFullList<AppSettingsRecord>();
      if (records.length > 0) {
        setRecord(records[0]);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    pb.collection('app_settings')
      .subscribe<AppSettingsRecord>('*', (event: RecordSubscription<AppSettingsRecord>) => {
